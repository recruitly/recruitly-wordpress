...
function recruitly_wordpress_truncate_post_type() {
	try {
		...
		$query = "SELECT ID FROM $wpdb->posts WHERE post_type = '$customPostType'";
...
function recruitly_wordpress_insert_post_type()
{
    ...
    recruitly_wordpress_sync_post_type(
        esc_html( get_option( 'recruitly_apikey' ) ),
        esc_url( get_option( 'recruitly_apiserver' ) ),
        esc_html(get_option( 'recruitly_customposttype','recruitlyjobs' ) )
    );
 ...
 *
 */
function recruitly_wordpress_sync_post_type($apiKey,$apiServer,$customPostType) {
	...
	try {
		...
		$queryRjobs = "SELECT ID FROM $wpdb->posts WHERE post_type ='$customPostType'";
		...
		if ( !empty( $restResponse->data ) && ! empty( $jobIdList ) ) {
        ...
        recruitly_admin_notice( "Recruitly job sync completed successfully for post type $customPostType!", 'success' );
